...
% =========================================================================
[ndof,X,connectivity,Ndof,Nelem,totaldof,E,A,I,H,F,U,alldof,Elastic,Bnode,w,forceunits,lengthunits,UDLFinternal,rho]=readframeputdata1011(datafilename);
...
[Finternal]=compute_internal_forces(Nelem,X,connectivity,E,A,I,U2,UDLFinternal,Ndof,alldof);
...
while true
    ...
    switch choice
        case 1
            while true
                user_input = input('Enter BMD scale (e.g. 0.01), or type "exit" to return: ', 's');
                if strcmpi(user_input, 'exit')
                    break;
                end
                scale = str2double(user_input);
                if isnan(scale) || scale <= 0
                    disp('Invalid input. Please enter a positive number or "exit".');
                else
        ...
        case 2
            while true
                user_input = input('Enter SFD scale (e.g. 0.01), or type "exit" to return: ', 's');
                if strcmpi(user_input, 'exit')
                    break;
                end
                scale = str2double(user_input);
        ...
        case 3
            while true
                user_input = input('Enter deformation scale (e.g. 1000), or type "exit" to return: ', 's');
                if strcmpi(user_input, 'exit')
                    break;
                end
                scale = str2double(user_input);
        ...
        case 8
            disp('--- Modal Analysis: Natural Frequencies and Mode Shapes ---');

            % Step 0: Ask user whether to use default or custom deformation scale
            disp('Deformation Scale Options:');
            disp('  1 - Use default scale factor (0.05)');
            disp('  2 - Enter custom scale factor');
            scale_option = input('Enter your choice [1/2]: ');

            if scale_option == 1
                ampl_default = 0.05;
            else
                user_input = input('Enter deformation scale factor (e.g., 0.03): ', 's');
                if isempty(user_input)
                    ampl_default = 0.05;
                else
                    ampl_default = str2double(user_input);
                    if isnan(ampl_default) || ampl_default <= 0
                        warning('Invalid input. Using default scale 0.05.');
                        ampl_default = 0.05;
                    end
                end
            end

            % Step 1: Compute the global mass matrix
            M = compute_mass_matrix(X, connectivity, Nelem, A, rho, I, totaldof);

            % Step 2: Reduce matrices to free DOFs
            ...
            [qbar, w2] = eig(A);
            w8 = diag(sqrt(w2));
            freq = real(w8 / (2 * pi));  % Convert to Hz and remove imaginary parts
            [freq, idx] = sort(freq);
            qbar = qbar(:, idx);         % Reorder mode shapes accordingly

            % Step 4: Ask user what to display
            disp(' ');
            disp('Display options:');
            disp('  1 - Show first 5 (or fewer) natural frequencies and mode shapes');
            disp('  2 - Show all natural frequencies and mode shapes');
            disp('  3 - Show a specific mode');
            choice = input('Enter your choice [1/2/3]: ');

            % Step 5: Common settings for plotting
            nnode8 = size(X, 1);
            ngdof = totaldof;
            xmax = max(X(:,1)); xmin = min(X(:,1));
            ymax = max(X(:,2)); ymin = min(X(:,2));
            maxsize = max([xmax - xmin, ymax - ymin]);

            % Build adjacency matrix for plotting
            adjacency = zeros(nnode8);
            for e8 = 1:size(connectivity, 1)
                i8 = connectivity(e8, 1);
                j8 = connectivity(e8, 2);
                adjacency(i8, j8) = 1;
                adjacency(j8, i8) = 1;
            end

            % Case 1 or 2: Multiple modes
            if choice == 1 || choice == 2
                if choice == 1
                    n_display = min(5, length(freq));
                else
                    n_display = length(freq);
                end

                % Display natural frequencies
                fprintf('\nNatural Frequencies (Hz):\n');
                for i1 = 1:n_display
                    fprintf('  Mode %2d:  %.6f Hz\n', i1, real(freq(i1)));
                end

                % Display modal shape matrix
                fprintf('\nModal Shape Matrix qbar (first %d modes):\n', n_display);
                disp(qbar(:, 1:n_display));
                disp('Note: Mode shapes are not normalized.');

                % Plot each mode shape
                figure('Name', 'Mode Shapes Visualization');
                for imode = 1:n_display
                    Umode = zeros(ngdof, 1);
                    Umode(freedof) = qbar(:, imode);
                    Umode(fixdof) = 0;

                    % Use Ndof mapping to extract displacements
                    Ux = Umode(Ndof(:,1));
                    Uy = Umode(Ndof(:,2));
                    maxU = max(abs([Ux; Uy]));
                    amplfact = maxsize / maxU * ampl_default;

                    defcoord = zeros(nnode8, 2);
                    defcoord(:,1) = X(:,1) + amplfact * Ux;
                    defcoord(:,2) = X(:,2) + amplfact * Uy;

                    subplot(ceil(sqrt(n_display)), ceil(sqrt(n_display)), imode);
                    gplot(adjacency, X, '-ob'); hold on;
                    gplot(adjacency, defcoord, '-or');
                    title(['Mode ', num2str(imode), ' (', num2str(real(freq(imode)), '%.4f'), ' Hz)']);
                    axis equal; grid on;
                    xlim([xmin xmax] + [-1 1]*0.2*maxsize);
                    ylim([ymin ymax] + [-1 1]*0.2*maxsize);
                end

                % Case 3: Specific mode
            elseif choice == 3
                mode_num = input(sprintf('Enter a mode number (1 to %d): ', length(freq)));
                if mode_num < 1 || mode_num > length(freq)
                    disp('Invalid mode number.');
                else
                    % Display mode info
                    fprintf('\nSelected Mode %d:\n', mode_num);
                    fprintf('  Natural Frequency: %.6f Hz\n', real(freq(mode_num)));
                    disp('  Corresponding mode shape vector (qbar):');
                    disp(qbar(:, mode_num));

                    % Construct full displacement vector
                    Umode = zeros(ngdof, 1);
                    Umode(freedof) = qbar(:, mode_num);
                    Umode(fixdof) = 0;
                    ...
                    figure('Name', ['Mode ', num2str(mode_num)]);
                    gplot(adjacency, X, '-ob'); hold on;
                    gplot(adjacency, defcoord, '-or');
                    title(['Mode ', num2str(mode_num), ' (', num2str(real(freq(mode_num)), '%.4f'), ' Hz)']);
                    axis equal; grid on;
                    xlim([xmin xmax] + [-1 1]*0.2*maxsize);
                    ylim([ymin ymax] + [-1 1]*0.2*maxsize);
                end
...
% =========================================================================================================================================
    ...
    function [M] = compute_mass_matrix(X, connectivity, Nelem, A, rho, I, totaldof)
        % COMPUTE_MASS_MATRIX - Compute global consistent mass matrix for 2D frame (Euler-Bernoulli beam)
        %
        % Inputs:
        %   X            : Node coordinates [nnode x 2]
        %   connectivity : Element connectivity [nelem x 2]
        %   Nelem        : DOF mapping per element [1 x 6 x nelem]
        %   A, rho, I    : Cross-sectional area, density, moment of inertia [nelem x 1]
        %   totaldof     : Total number of DOFs (scalar)
        %
        % Output:
        %   M            : Global mass matrix [totaldof x totaldof]

        M = zeros(totaldof, totaldof);
        ...
        for ielem = 1:nelem
            ...
            A_e   = A(ielem);
            rho_e = rho(ielem);
            I_e   = I(ielem);  %#ok<NASGU> % I_e not used here but kept for interface compatibility



            
            % Compute local consistent mass matrix directly (6x6 for Euler-Bernoulli beam)
            m = rho_e * A_e * L;
            Me_local = m * [
                1/3      0          0          1/6       0           0;
            ...
            M(edof, edof) = M(edof, edof) + Me_local;
        end
    end

    function [Finternal]=compute_internal_forces(Nelem,X,connectivity,E,A,I,U2,UDLFinternal,Ndof,alldof)
        %initial allocation
  ...
  % Function to plot Bending Moment and Shear Force with scaling
    ...
    function [TotalYd, TotalXd, TotalGlobal] = plot_deformation_local(X, connectivity, U, Ud, Nelem, w, E, I, A, Finternal, fixdof, UFINAL)
        ...
        num_elem = size(connectivity,1);
        TotalYd = zeros(11, num_elem);       % 局部 y（挠度）
        TotalXd = zeros(11, num_elem);       % 局部 x（轴向位移）
        TotalGlobal = zeros(11, 2, num_elem);% 全局 ΔX, ΔY 位移
        ...
        for ielem = 1:num_elem
            ...
            u1 = U1_local(1); u2 = U2_local(1);

            % === 垂向挠度变换 ===
            T = [c s 0; -s c 0; 0 0 1];
            T2 = [T, T; T, T];
            Udx = U;
            x_start = Udx(idof(1));x_end = Udx(jdof(1));
            ...
            for k = 1:11
                ...
                TotalGlobal(k,:,ielem) = global_disp';  % 存储 ΔX, ΔY
            end

            % === 首尾精确值修正 ===

            TotalGlobal(1,1,ielem) = x_start; TotalGlobal(end,1,ielem) = x_end;
            Xd(1) = u1;  Xd(end) = u2;
            Yd(1) = ya;  Yd(end) = yb;
