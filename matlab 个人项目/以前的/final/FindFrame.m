...
% Function to plot Bending Moment and Shear Force with scaling
    ...
    function BM_matrix = plot_BM_only(X, connectivity, Finternal, w, scale, draw)
        ...
        if draw
            ...
            % Calculate padding (10% of the total range)
            xpad = 0.02 * (xlimits(2) - xlimits(1));
            ypad = 0.02 * (ylimits(2) - ylimits(1));

            % Apply padded axis limits
    ...
    function [TotalYd] = plot_deformation(X,connectivity,U,Ud,Nelem,w,E,I,A,Finternal,fixdof)

        TotalYd = zeros(11,size(connectivity,1));
        ...
        for ielem=1:size(connectivity,1)
            ...
            Ud(Nelem(:,1:3,ielem)) = T2(1:3,1:3)*U(Nelem(:,1:3,ielem));
            Ud(Nelem(:,4:6,ielem)) = T2(4:6,4:6)*U(Nelem(:,4:6,ielem));
            % Ud(Nelem(:,1:3,ielem)) = T*U(Nelem(:,1:3,ielem));
            % Ud(Nelem(:,4:6,ielem)) = T*U(Nelem(:,4:6,ielem));
            ...
            Mb  = [yb-ya-ra*L-(w(ielem)*L^4/(24 * E(ielem)*I(ielem)));rb-ra-(w(ielem)*L^3/(6 * E(ielem)*I(ielem)))];
            % sovle
            Rab = MA \ Mb;
            % output
            a = Rab(1);
