...
% =========================================================================================================================================
    ...
    function BM_matrix = plot_BM_only(X, connectivity, Finternal, w, scale, draw)
        ...
        % Dictionary to avoid duplicate annotations at shared nodes
        ...
        for e = 1:num_elem
            ...
            R_bm = [cx, -cy; cy, cx];  % Rotation matrix
            
            MA = Finternal(e,1,3);  % Moment at node A
            ...
            if draw
                % Plot element and bending moment curve
                ...
                for k = 1:length(x_bm_local)
                    ...
                    plot([p0(1), p1(1)], [p0(2), p1(2)], 'r--');  % Dotted vertical lines
                end

                % End vertical line (with epsilon offset)
                p0_tail = R_bm * [L_elem; 0] + [xn1; yn1];
                p1_tail = R_bm * [L_elem; offset_bm(end) + epsilon] + [xn1; yn1];
                plot([p0_tail(1), p1_tail(1)], [p0_tail(2), p1_tail(2)], 'r--');

                % Annotate values at start, middle, and end
                idxs = [1, 6, 11];  % start, mid, end
                node_ids = [n1, -1, n2];  % -1 means no node ID (midpoint)
                ...
                for j = 1:3
                    ...
                    pos = pts_global(:,j);
                    if j == 2
                        % Midpoint — always show with circle and label
        ...
        if draw
            % Automatically expand axis limits slightly
            xlimits = xlim;
            ylimits = ylim;
